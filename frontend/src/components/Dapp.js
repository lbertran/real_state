...
export class Dapp extends React.Component {
  ...
  render() {
    ...
    // a loading component.
    if (!this.state.assetsCollection) {
      return <Loading />;
    }
  ...
  async _initializeEthers() {
    ...
    this._assetFactory = new ethers.Contract(
      contractAddress.AssetFactory,
      AssetFactoryArtifact.abi,
      this._provider.getSigner(0)
    );

    this._lendingBorrowingFactory = new ethers.Contract(
      contractAddress.LendingBorrowingFactory,
      LendingBorrowingFactoryArtifact.abi,
      this._provider.getSigner(0)
    );

    this._priceConsumer = new ethers.Contract(
      contractAddress.PriceConsumer,
  ...
  async _getAssetsCollection() {
    const _assetsCollection = await this._assetFactory.allAssets();

    this.setState({ assetsCollection: _assetsCollection });
  }
